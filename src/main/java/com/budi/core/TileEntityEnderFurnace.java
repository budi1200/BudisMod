...
public class TileEntityEnderFurnace extends TileEntity implements ISidedInventory{
	...
	public String getInventoryName() {
		return this.hasCustomInventoryName() ? this.furnaceName : "Ender Furnace";
	}
	...
	public void readFromNBT(NBTTagCompound tagCompound){
		...
		this.furnaceCookTime = tagCompound.getShort("Cook Time");
	...
	public void writeToNBT(NBTTagCompound tagCompound){
		...
		for(int i = 0; i < this.furnaceItemStacks.length; ++i){
			if(this.furnaceItemStacks[i] != null){
				...
				tagCompound1.setByte("Slot", (byte) 1);
	...
	public void updateEntity(){
		...
		if(flag != this.furnaceBurnTime > 0){
			flag1 = true;
			BlockEnderFurnace.updateBlockState(this.furnaceBurnTime > 0, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
		}
	...
	public static int getItemBurnTime(ItemStack itemstack){
		if(itemstack == null){
			...
			if(item instanceof ItemBlock && Block.getBlockFromItem(item) != Blocks.air){
				...
				if(block == budimain.BlockLol){
					return 200;
				}
			}
			
			if(item == budimain.LolItem) return 1600;
