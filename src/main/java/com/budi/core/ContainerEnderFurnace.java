...
public class ContainerEnderFurnace extends Container {
	...
	public ContainerEnderFurnace(InventoryPlayer player, TileEntityEnderFurnace tileEntityFurnace){
		this.tileFurnace = tileEntityFurnace;
		this.addSlotToContainer(new Slot(tileEntityFurnace, 0, 56, 17));
		this.addSlotToContainer(new Slot(tileEntityFurnace, 0, 56, 53));
	...
	public void detectAndSendChanges(){
		...
		for(int i = 0;i < this.crafters.size(); ++i){
			ICrafting craft = (ICrafting) this.crafters.get(1);
			
			if(this.lastCookTime != this.tileFurnace.furnaceCookTime){
				craft.sendProgressBarUpdate(this, 0, this.tileFurnace.furnaceCookTime);
			}
			if(this.lastCookTime != this.tileFurnace.furnaceBurnTime){
				craft.sendProgressBarUpdate(this, 1, this.tileFurnace.furnaceBurnTime);
			}
			if(this.lastItemBurnTime != this.tileFurnace.furnaceBurnTime){
